...
public static string GetWord(string input, string seed)
{
    ...
    for (int i = 0; i < firstArray.Length; i++)
        if (firstArray[i].Length > wordWithMaxLength.Length)
            wordWithMaxLength = firstArray[i];

    if (wordWithMaxLength.Length < seed.Length)
        wordWithMaxLength = seed;

    int indexOfA = wordWithMaxLength.IndexOf('a');

    if (indexOfA != -1)
    {
        return wordWithMaxLength.Substring(indexOfA);
    }
    else
    {
        return string.Empty;
