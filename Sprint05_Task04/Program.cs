...
class MyUtils
{
    ...
    {
        List<string> combinedList = list.Distinct().ToList();

        List<string> uniqueDictionaryValues = dictionary.Values.Distinct().ToList();

        combinedList.Sort();
        uniqueDictionaryValues.Sort();

        bool areListsEqual = combinedList.SequenceEqual(uniqueDictionaryValues);

        return areListsEqual || (combinedList.Count == 0 && uniqueDictionaryValues.Count == 0);
    }
}
